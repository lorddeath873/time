...
function pwgm()
{
    ...
    $zah=range(0, 9);
    $so=array('@','?','!','$'); //ggf. Erweitern
    $z_p=array_merge($bst, $zah, $so);
    $z_p=array_flip($z_p); //Damit array-Rand den Array-Wert und nicht den Array-Index bekommt
    $lä=rand(8, 12);
    $pass='';
    for ($x=1; $x<$lä; $x++) {
        $pass.=array_rand($z_p);
...
if ($_SESSION['usgrp'] == "1" OR $_SESSION['usgrp'] == "2" && isset($_SESSION['mid'])) {
    ...
    <tr>
    <td class="textbox"><? echo $locate ['106'] ?></td>
    <td class="table-body"><input name="pw" type="password" value="<? echo $pwm ?>" readonly="readonly" /></td>
    ...
    <tr>
    <td class="textbox"><? echo $locate ['112'] ?></td>
    ...
    <tr>
    <td class="textbox"><? echo $locate ['147'] ?></td>
    ...
    <td class="table-body"><input name="me" type="text" value="<? echo $_SESSION['mid'] ?>" maxlength="10" readonly="readonly"></td>
    <td class="textbox"><? echo $locate ['121'] ?></td>
    </tr>
    <tr>
    ...
    if (isset($_REQUEST['ok'])) {
		...
		$stmt = $mysqli->prepare("INSERT INTO usr_login (ma, pw, lo, logrp, name, surname, geb, ein, aus, street, plz, tel, mob, ort, url, reurl, usrgrp, ber, me, mail, fid) 
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
		$stmt->bind_param('ssssssssssissssssssss',$_POST['ma'], $pwm5, $_POST['lo'], $_POST['logrp'], $_POST['name'], $_POST['surname'], $_POST['geb'], $_POST['ein'], $_POST['aus'], $_POST['street'], $_POST['plz'], $_POST['tel'], $_POST['mob'], $_POST['ort'], $_POST['url'], $_POST['reurl'], $_POST['usrgrp'], $_POST['ber'], $_POST['me'], $_POST['mail'], $_POST['fid']);
		$stmt->execute();
		$stmt->close();
